...
public class AlbumDAO extends DAO {
	...
	public static int movePhoto(int siteid, AlbumBean fromCat,AlbumBean toCat){
		...
		try {
			...
			if(fromCat.getParent()==null || fromCat.getParent().getId()!=toCat.getId()){
				//4. 修改目标相簿的照片数
			...
			if(toCat.getParent()==null || toCat.getParent().getId()!=fromCat.getId())
				//7. 重新计算源相簿的照片数
	...
	public static void delete(int siteid, int album_id){
		commitNamedUpdate("DELETE_ALBUM", siteid, album_id);
	}
}
