...
public class DiaryDAO extends DAO implements SearchDataProvider {
	...
	private final static Log log = LogFactory.getLog(DiaryDAO.class);
	...
	public static int markDiaryAsElite(int diary_id, boolean elite) {
		return commitNamedUpdate("MODIFY_TYPE_OF_DIARY",
				elite ? DiaryBean.TYPE_ELITE : 0, diary_id);
	}
	...
	public static List listNewArticles(int fromIdx, int count) {
		String key = fromIdx + "_" + count;
		List logs = (List) getObjectCached("query.new_diarys", key);
		if (logs == null) {
			logs = executeNamedQuery("LIST_NEW_DIARY", fromIdx, count,
					DiaryOutlineBean.STATUS_NORMAL, CatalogBean.TYPE_OWNER);
			if (logs != null)
				setObjectCached("query.new_diarys", key, logs);
		}
		return logs;
	}
	...
	public static List listHotArticles(int days, int count) {
		...
		return executeNamedQueryCacheable("query.hot_diarys",
				"LIST_HOT_DIARY2", 0, count, DiaryOutlineBean.STATUS_NORMAL,
				cal.getTime(), CatalogBean.TYPE_OWNER);
	}
	 ...
	 * @return
	 */
	public static List listRecommendArticles(int fromIdx, int count) {
		String cache = "query.rec_diarys";
		String key = fromIdx + "_" + count;
		List logs = (List) getObjectCached(cache, key);
		if (logs == null) {
			logs = executeNamedQuery("LIST_RECOMMEND_ARTICLES", fromIdx, count,
					DiaryBean.TYPE_ELITE, DiaryBean.STATUS_NORMAL,
					CatalogBean.TYPE_OWNER);
			if (logs != null)
				setObjectCached(cache, key, logs);
		}
		return logs;
	}
	...
	public static List listHotArticles(int siteid, int days, int count) {
		String cache = "query.hot_diarys";
		String key = siteid + "_" + days + "_" + count;
		List logs = (List) getObjectCached(cache, key);
		if (logs == null) {
			Calendar cal = Calendar.getInstance();
			DateUtils.resetTime(cal);
			cal.add(Calendar.DATE, -days);
			logs = executeNamedQuery("LIST_HOT_DIARY", 0, count, siteid,
	...
	public static void unDelete(DiaryOutlineBean log) {
		...
		try {
			...
			ssn.update(log);
			commit();
			DAO.evict(DiaryBean.class, log.getId());
	 ...
	 * @return
	 */
	public static int[] statCalendarLogs(SiteBean site, SessionUserObject user,
			Calendar month) {
		...
		StringBuffer hql = new StringBuffer(
				"SELECT j.writeTime FROM DiaryBean AS j WHERE j.writeTime>=:beginTime AND j.writeTime<:endTime AND j.status=:status AND j.site.id=:site");
		if (!site.isOwner(user)) {
			// 排除用户没有权限访问的分类
			hql.append(" AND (j.catalog.type<>:cat_type");
			if (user != null)
				hql
						.append(" OR (j.catalog.type=:cat_type AND j.catalog.id IN (SELECT p.key.catalog FROM CatalogPermBean AS p WHERE p.key.user=:user))");
			hql.append(')');
		}
		...
		try {
			...
			q.setCacheable(true).setCacheRegion("query.diary_calendar");
			int total = 0;
	...
	public static int getTrashCount(int site_id) {
		return executeNamedStat("DIARY_COUNT_BY_STATUS", site_id,
				DiaryBean.STATUS_DELETED).intValue();
	}
	...
	public static List listTrash(int site_id, int fromIdx, int count) {
		return executeNamedQuery("LIST_DIARY_BY_STATUS", fromIdx, count,
				site_id, DiaryBean.STATUS_DELETED);
	}
	...
	public static int getDraftCount(SiteBean site, int userid) {
		return executeNamedStat("DRAFT_COUNT", site.getId(), userid,
				DiaryBean.STATUS_DRAFT).intValue();
	}
	...
	public static List listDrafts(SiteBean site, int userid, int fromIdx,
			int count) {
		return listDrafts(site, userid, fromIdx, count, true);
	}

	public static List listDrafts(SiteBean site, int userid, int fromIdx,
			int count, boolean withContent) {
		String hql = withContent ? "LIST_DRAFT" : "LIST_DRAFT_OUTLINE";
		return executeNamedQuery(hql, fromIdx, count, site.getId(), userid,
				DiaryBean.STATUS_DRAFT);
	}
	...
	public static void incViewCount(int log_id, int incCount) {
		String sql = "update dlog_diary set view_count=view_count+?, last_read_time=? where diary_id=?";
		try {
			commitSQLUpdate(sql, incCount, new Timestamp(System
					.currentTimeMillis()), log_id);
		} catch (SQLException e) {
			log.error("Exception occur when inc diary viewcount", e);
		}
	}
	...
	public static void forceDelete(int log_id) throws Exception {
		Session ssn = getSession();
		try {
			DiaryBean log = (DiaryBean) ssn.load(DiaryBean.class, new Integer(
					log_id));
			beginTransaction();
			// 如果是正常日记则对应的分类日记数减一
			...
			// 删除附件
			FCKUploadFileDAO.deleteFilesByRef(ssn, log.getSite().getId(),
					log_id, DiaryBean.TYPE_DIARY);
			List list = AnnexDAO.findByDiaryId(log_id);
			// 删除磁盘文件
			int diaryid = 0;
			for (int i = 0; i < list.size(); i++) {
				AnnexBean bean = (AnnexBean) list.get(i);
				diaryid = bean.getDiaryID();
				File f = new File(bean.getDiskPath());
				if (f != null)
					f.delete();
			}
			StringBuffer hql = new StringBuffer();
			hql.append(" DELETE AnnexBean as a ");
			hql.append(" WHERE  a.diaryID=? ");
			executeUpdate(hql.toString(), diaryid);
			// 所有参与该相片评论者的相册评论数减一
	...
	public static void cleanupTrash(int site_id) throws Exception {
		List logs = findNamedAll("QUERY_TRASH_BEFORE_CLEANUP", site_id,
				DiaryBean.STATUS_DELETED);
		if (logs != null && logs.size() > 0) {
			try {
				...
				for (int i = 0; i < logs.size(); i++) {
					...
					// 删除附件
					FCKUploadFileDAO.deleteFilesByRef(ssn, site_id,
							log.getId(), DiaryBean.TYPE_DIARY);
					// 删除日记
					ssn.delete(log);
				}
				commit();
	 ...
	 * @return
	 */
	public static boolean canUserEditDiary(SessionUserObject user,
			DiaryBean diary) {
		if (user == null || diary == null
				|| user.getStatus() != UserBean.STATUS_NORMAL)
			return false;
	 ...
	 * @return
	 */
	public static DiaryOutlineBean getPrevDiary(SiteBean site,
			SessionUserObject user, int cat_id, int log_id) {
		if (site == null)
			return null;
		StringBuffer hql = new StringBuffer(
				"FROM DiaryOutlineBean AS j WHERE j.site.id=:site AND j.status=:status AND j.id<:diary");
		if (!site.isOwner(user)) {
			// 排除用户没有权限访问的分类
			hql.append(" AND (j.catalog.type<>:type");
			if (user != null)
				hql
						.append(" OR (j.catalog.type=:type AND j.catalog.id IN (SELECT p.key.catalog FROM CatalogPermBean AS p WHERE p.key.user=:user))");
			hql.append(')');
		}
		if (cat_id > 0) {
	 ...
	 * @return
	 */
	public static DiaryOutlineBean getNextDiary(SiteBean site,
			SessionUserObject user, int cat_id, int log_id) {
		if (site == null)
			return null;
		StringBuffer hql = new StringBuffer(
	...
	public static void update(DiaryBean diary, boolean updateTags) {
		try {
			...
			commit();
			evict(DiaryBean.class, diary.getId());
			evict(DiaryOutlineBean.class, diary.getId());
		} catch (HibernateException e) {
			rollback();
			throw e;
		}
	...
	public static int getPublicDiaryCount() {
		return executeNamedStat("PUBLIC_DIARY_COUNT", DiaryBean.STATUS_NORMAL,
				CatalogBean.TYPE_OWNER).intValue();
	}
	 ...
	 * @return
	 */
	public static int getDiaryCount(SiteBean site, SessionUserObject user,
			int catalog_id, int year, int month, int date) {
		StringBuffer hql = new StringBuffer(
				"SELECT COUNT(*) FROM DiaryBean AS a WHERE a.status=:status AND a.site.id=:site");
		if (!site.isOwner(user)) {
			// 排除用户没有权限访问的分类
			hql.append(" AND (a.catalog.type<>:cat_type");
			if (user != null)
				hql
						.append(" OR (a.catalog.type=:cat_type AND a.catalog.id IN (SELECT p.key.catalog FROM CatalogPermBean AS p WHERE p.key.user=:user))");
			hql.append(')');
		}
		if (catalog_id > 0)
		...
		if (year > 0 || month > 0 || date > 0) {
			hql
					.append(" AND a.writeTime >= :beginTime AND a.writeTime < :endTime");
		}
		try {
			Session ssn = getSession();
			Query q = ssn.createQuery(hql.toString());
	 ...
	 * @return
	 */
	public static List listDiary(int year, int month, int date, int fromIdx,
			int count, boolean withContent) {
	 ...
	 * @return
	 */
	public static List listDiary(SiteBean site, SessionUserObject user,
			int catalog_id, int year, int month, int date, int fromIdx,
			int count, boolean withContent) {
		StringBuffer hql = new StringBuffer("FROM ");
		hql.append(withContent ? "DiaryBean" : "DiaryOutlineBean");
	 ...
	 * @return
	 */
	public static List listDiaryReplies(SiteBean site, int fromIdx, int count,
		...
		if (!site.isOwner(user)) {
			...
			if (user != null)
				hql
						.append(" OR (r.diary.catalog.type=:cat_type AND r.diary.catalog.id IN (SELECT p.key.catalog FROM CatalogPermBean AS p WHERE p.key.user=:userid))");
			hql.append(')');
	 ...
	 * @return
	 */
	public static List listDiaryReplies(int log_id, int fromIdx, int count,
			boolean reverse) {
		String hql_name = reverse ? "LIST_REPLIES_OF_DIARY"
				: "LIST_REPLIES_OF_DIARY2";
	...
	public static List listDiaryRepliesAfter(Date date) {
		return findNamedAll("LIST_DIARY_REPLIES", date,
				_ReplyBean.STATUS_NORMAL, CatalogBean.TYPE_OWNER);
	}
	...
	public static int getDiaryReplyCount(SiteBean site, SessionUserObject user) {
		...
		if (!site.isOwner(user)) {
			...
			if (user != null)
				hql
						.append(" OR (r.diary.catalog.type=? AND r.diary.catalog.id IN (SELECT p.key.catalog FROM CatalogPermBean AS p WHERE p.key.user=?))");
			hql.append(')');
		}
		Session ssn = getSession();
		Query q = ssn.createQuery(hql.toString());
	...
	/**
	 * 创建日记评论,自动更新对应日记的评论数 当评论数超过最大的允许评论数后自动锁贴
	 * 
	 * @param reply
	 */
	public static void createDiaryReply(DiaryReplyBean reply) {
		try {
			Session ssn = getSession();
			int max_reply_count = ConfigDAO.getMaxReplyCount(reply.getSite()
					.getId());
			beginTransaction();
			reply.getDiary().incReplyCount(1);
			if (reply.getDiary().getReplyCount() >= max_reply_count
					&& max_reply_count > 0)
	...
	public static boolean MoveDiary(List list) {
		System.out.println("MoveDiary");
		try {
			Session ssn = getSession();
			for (int i = 0; i < list.size(); i++) {
				beginTransaction();
				DiaryBean bean = (DiaryBean) list.get(i);
				ssn.save(bean);
				flush();
				commit();
			}
