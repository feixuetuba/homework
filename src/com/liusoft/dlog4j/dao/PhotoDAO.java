...
public class PhotoDAO extends DAO implements SearchDataProvider {
	...
	public static List listAlbums(int site_id, AlbumBean parent) {
		StringBuffer hql = new StringBuffer(
				"FROM AlbumBean a WHERE a.site.id=?");
		if (parent != null) {
	...
	public static List listHotPhotos(int days, int count) {
		...
		return executeNamedQueryCacheable("query.hot_photos",
				"LIST_HOT_PHOTOS", 0, count, PhotoOutlineBean.STATUS_NORMAL,
				cal.getTime(), AlbumBean.TYPE_PUBLIC);
	}
	 ...
	 * @return
	 */
	public static PhotoOutlineBean getPrevPhoto(SiteBean site,
			SessionUserObject user, int album_id, int photo_id) {
		if (site == null)
			return null;
	 ...
	 * @return
	 */
	public static PhotoOutlineBean getNextPhoto(SiteBean site,
			SessionUserObject user, int album_id, int photo_id) {
		if (site == null)
			return null;
		StringBuffer hql = new StringBuffer(
	 ...
	 *      int, int)
	 */
	public static List listPhotos(SiteBean site, SessionUserObject user,
			int album_id, int month_stamp, int date, int fromIdx, int count) {
		StringBuffer hql = new StringBuffer(
				"FROM PhotoOutlineBean AS p WHERE p.site.id=:site");
		if (album_id > 0)
		...
		if (user == null || site.getOwner().getId() != user.getId()) {
			hql
					.append(" AND p.status<>:hidden_status AND p.album.type=:owner_album");
		}
		if (date > 0) {
	 ...
	 *      int, int)
	 */
	public static List listPhotos(int album_id, int month_stamp, int date,
			int fromIdx, int count) {
		StringBuffer hql = new StringBuffer(
				"FROM PhotoOutlineBean AS p WHERE 1=1");
		if (album_id > 0)
			hql.append(" AND (p.album.id=:album OR p.album.parent.id=:album)");
		else {
	 ...
	 *      int, int)
	 */
	public static int getPhotoCount(SiteBean site, SessionUserObject user,
			int album_id, int month_stamp, int date) {
		boolean is_owner = site.isOwner(user);
		StringBuffer hql = new StringBuffer(
				"SELECT COUNT(*) FROM PhotoBean AS p WHERE p.site.id=:site");
		if (album_id > 0)
			hql.append(" AND p.album.id=:album");
		if (month_stamp > 190000 && month_stamp < 209912) {
			hql.append(" AND p.year=:year AND p.month=:month");
		...
		if (!is_owner) {
			hql
					.append(" AND p.status=:normal_status AND p.album.type=:public_album");
		}
		if (date > 0) {
			hql.append(" AND p.date=:date");
		}
	...
	public static void delete(_PhotoBase photo) throws SQLException {
		...
		try {
			beginTransaction();
			// 释放相册空间
			int photo_size = DLOG4JUtils.sizeInKbytes(photo.getPhotoInfo()
					.getSize());
			...
			// 修改设置该照片为封面的相簿
			executeUpdate(
					"UPDATE AlbumBean AS a SET a.cover = NULL WHERE a.cover.id=?",
					photo.getId());
	 ...
	 */
	public static void update(int new_album_id, PhotoBean photo,
			String newKeyword, boolean cover) throws ObjectNotFoundException,
		...
		try {
			...
			if (photo.getAlbum().getType() == AlbumBean.TYPE_PUBLIC
				...
				if (!StringUtils.equals(photo.getKeyword(), newKeyword)) {
					...
					TagDAO
							.deleteTagByRefId(photo.getId(),
									DiaryBean.TYPE_PHOTO);
	...
	public static void create(AlbumBean album, PhotoBean photo, boolean cover)
		...
		try {
			beginTransaction();
			// 修改site的已用相册空间
			int photo_site = DLOG4JUtils.sizeInKbytes(photo.getPhotoInfo()
					.getSize());
	...
	public static int getTotalPhotoSize(int sid, int album_id) {
		if (sid < 1)
			return -1;
		StringBuffer hql = new StringBuffer(
				"SELECT SUM(p.photoInfo.size) FROM PhotoBean AS p WHERE p.site.id=?");
		if (album_id > 0)
	...
	/**
	 * 统计指定月份每天的照片数 FIXME: 此方法的缓存对单独页面有效，但是放在photo/show.vm中就无效，原因不详
	 * 
	 * @param site
	 * @param loginUser
	 * @param month
	 * @return
	 */
	public static int[] statCalendarPhotoCount(SiteBean site,
			SessionUserObject user, Calendar month) {
	...
	public static int getPhotoReplyCount(SiteBean site, SessionUserObject user) {
		...
		if (!is_owner) {
			...
			return executeStatAsInt(hql.toString(),
					PhotoReplyBean.STATUS_NORMAL, site.getId(),
					AlbumBean.TYPE_PUBLIC, PhotoBean.STATUS_NORMAL);
		}
		return executeStatAsInt(hql.toString(), PhotoReplyBean.STATUS_NORMAL,
				site.getId());
	}
	 ...
	 * @return
	 */
	public static List listPhotoReplies(SiteBean site, int fromIdx, int count,
	...
	/**
	 * 创建照片评论,自动更新对应照片的评论数 当评论数超过最大的允许评论数后自动锁贴
	 * 
	 * @param reply
	 */
	...
	public static List diaryRelating(SiteBean bean, int fromidx, int page) {
		...
		hql
				.append(" SELECT f.id,( SELECT d.title FROM DiaryBean as d WHERE d.id=f.refId),f.uri,f.site.id ");
	...
	public static int diaryRelatingCount(SiteBean bean) {
		...
		hql.append(" FROM FckUploadFileBean as fub");
		hql.append(" WHERE fub.site.id = ?");
		int count = Integer.parseInt(uniqueResult(hql.toString(), bean.getId())
				.toString());
		return count;
	...
	public static List list_albums_view(SiteBean bean) {
		...
		FckUploadFileBean fBean = (FckUploadFileBean) uniqueResult(
				hql2.toString(), bean.getId());
		//贴图数	
