...
public class DLOG_Photo_VelocityTool extends VelocityTool {
	...
	public PhotoReplyBean reply(int reply_id) {
		if (reply_id < 0)
			return null;
		return (PhotoReplyBean) ReplyDAO.getReply(PhotoReplyBean.class,
				reply_id);
	}
	...
	public List replies(SiteBean site, SessionUserObject user,
			_PhotoBase photo, int page, int pageSize) {
	 ...
	 * @return
	 */
	public List list_photos(SiteBean site, SessionUserObject user,
			int album_id, int month_stamp, int page, int count) {
		if (count < 1)
			count = 10;
		int fromIdx = (page - 1) * count;
		if (fromIdx < 0)
		...
		if (site == null)
			return PhotoDAO.listPhotos(album_id, month_stamp, date, fromIdx,
					count);
		else {
			if (album_id < 1)
				return PhotoDAO.listPhotos(site, user, album_id, month_stamp,
						date, fromIdx, count);
			else {
				// 判断用户有没有访问该相簿的权限
	 ...
	 * @return
	 */
	public List photo_counts_by_month(SiteBean site, SessionUserObject user,
			int year, int month) {
		if (site == null)
			return null;
	 ...
	 * @return
	 */
	public List list_photo_replies(SiteBean site, int page, int count,
	 ...
	 * @return
	 */
	public PhotoOutlineBean prev_photo(SiteBean site, SessionUserObject user,
			int album_id, int photo_id) {
	 ...
	 * @return
	 */
	public PhotoOutlineBean next_photo(SiteBean site, SessionUserObject user,
			int album_id, int photo_id) {
		if (site == null || photo_id < 0)
			return null;
		try {
	 ...
	 * @param log
	 */
	public void visit_photo(SiteBean site, SessionUserObject user,
			_PhotoBase photo) {
	 ...
	 * @return
	 */
	protected boolean canViewThisPhoto(SiteBean site, SessionUserObject user,
			_PhotoBase photo, HttpServletRequest req) {
		if (photo == null || photo.getSite().getId() != site.getId())
			return false;
		// 判断用户是否有访问的权限
		// 1. 站长可以访问任何属于他照片
		...
		if (photo.getAlbum().getType() == AlbumBean.TYPE_VERIFIED) {
			...
			if (ssn != null
					&& ssn.getAttribute("ALBUM_" + photo.getAlbum().getId()) != null) {
				return true;
			}
		}
		return false;
	 ...
	 * @return
	 */
	public boolean can_access_album(SiteBean site, SessionUserObject user,
			AlbumBean album, HttpServletRequest req) {
		...
		if (album.getType() == AlbumBean.TYPE_VERIFIED) {
			...
			HttpSession ssn = req.getSession(false);
			if (ssn != null
					&& ssn.getAttribute(Globals.ALBUM_VERIFY_KEY
							+ album.getId()) != null) {
				return true;
			}
		}
	 ...
	 * @return
	 */
	public int photo_count(SiteBean site, SessionUserObject user, int album_id,
			int month_stamp) {
		if (site == null)
			return -1;
		int year = RequestUtils.getParam(request, "year", -1);
	...
	public List format_photo_months(List months) {
		...
		if (months.size() > 0) {
			...
			final SimpleDateFormat month_fm = new SimpleDateFormat("MMM yyyy",
					request.getLocale());
	...
	public List list_albums(SiteBean site, AlbumBean parent) {
		if (site == null)
			return null;
		return PhotoDAO.listAlbums(site.getId(), parent);
	}
	 ...
	 * 照片总数
	 * */
	public int diaryRelatingCount(SiteBean bean) {
		
		return PhotoDAO.diaryRelatingCount(bean);
	 ...
	 * 照片列表
	 * */
	public List diaryRelating(SiteBean bean, int page, int count) {
		int fromidx = (page - 1) * count;
		List list = PhotoDAO.diaryRelating(bean,fromidx,count);
		return list;
	}
	 ...
	 * 相册列表
	 * */
	public List list_albums_view(SiteBean bean)	{
		return PhotoDAO.list_albums_view(bean);
	}
}
