...
public class DLOG_VelocityTool extends VelocityTool {
	...
	public List random(List objs, int count) {
		Collections.sort(objs, Globals.random_comparator);
		return objs.subList(0, Math.min(count, objs.size()));
	}
	 ...
	 * @param req
	 */
	public void save_refer(SiteBean site, int ref_id, int ref_type,
		...
		if (StringUtils.isNotEmpty(refer)) {
			...
			try {
				...
				log.error("Save refer failed, ip=" + req.getRemoteAddr()
						+ ",refer=" + refer, e);
			} finally {
				url = null;
			}
	...
	public boolean can_access_site(SiteBean site, UserBean loginUser) {
		...
		case SiteBean.ACCESS_MODE_SECRET_CODE:
			...
			if (ssn != null) {
				String pwd = (String) ssn.getAttribute("ACCESS_CODE_OF_SITE#"
						+ site.getId());
				return pwd != null
						&& StringUtils.equals(site.getAccessCode(), pwd);
			}
		}
		return false;
	}
	 ...
	 */
	public SiteBean site(int site_id) throws SiteNotFoundException {
		if (site_id <= 0)
			return null;
		...
		if (site == null || site.getStatus() < 0)
			throw new SiteNotFoundException(site_id);
		return site;
	}
	...
	/**
	 * 实现通过uuid参数进行自动登录,用于wap的书签自动登录 Added by Winter Lau , 2006-10-21
	 * 
	 * @return
	 */
	public SessionUserObject login_via_uuid() {
		String uuid = request.getParameter("uuid");
		return UserLoginManager.getUserByUUID(request, response, uuid);
	}
	 ...
	 * @return
	 */
	public String make_date_params(String param_name, int cat_id, int year,
			int month, int date) {
	 ...
	 * @param user
	 */
	public void visitor(SiteBean site, SessionUserObject user) {
		if (site == null || user == null)
			return;
		if (site.getOwner().getId() == user.getId())
			return;
		SiteDAO.visitor(site.getOwner().getId(), user.getId());
	}
	 ...
	 * @param user
	 */
	public List userVisitorList(SiteBean site, SessionUserObject user) {
		if (site == null || user == null)
			return null;
		return SiteDAO.userVisitorList(site.getOwner().getId());
