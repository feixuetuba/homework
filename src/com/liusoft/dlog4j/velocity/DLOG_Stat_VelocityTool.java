...
public class DLOG_Stat_VelocityTool{
	...
	public SiteStatInfo get_site_stat_info(SiteBean site) {
		...
		SiteStatInfo ssi = (SiteStatInfo) DLOG_CacheManager.getObjectCached(CACHE_KEY, site.getId());
		...
		if (ssi == null) {
			...
			DLOG_CacheManager.putObjectCached(CACHE_KEY, site.getId(), ssi);
		}
		return ssi;
	}
	...
	/**
	 * 记录页面访问统计
	 * TODO: 此处有必要进行改造，否则cookie太长
	 * @param site
	 */
	public boolean execute(HttpServletRequest request,
			HttpServletResponse response, SiteBean site, int source) {
		String ua = RequestUtils.getHeader(request, "user-agent");
		if(ua!=null&&(ua.indexOf("baidu")!=-1||ua.indexOf("google")!=-1||ua.indexOf("sogou")!=-1||ua.indexOf("sina")!=-1||ua.indexOf("iaskspider")!=-1||ua.indexOf("ia_archiver")!=-1||ua.indexOf("yahoo")!=-1||ua.indexOf("yodao")!=-1))
			return false;
