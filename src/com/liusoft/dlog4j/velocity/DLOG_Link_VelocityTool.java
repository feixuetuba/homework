...
public class DLOG_Link_VelocityTool extends StrutsLinkTool {
	 ...
	 * @see /wml/links.vm
	 */
	public List links_for_wap(SiteBean site, int page, int count){
		if(site == null)
			return null;
		int fromIdx = -1;
		if(page>0&&count>0)
			fromIdx = (page - 1) * count;
		return LinkDAO.getLinksOfWapSite(site, fromIdx, count);
	}
	 ...
	 * @see /wml/links.vm
	 */
	public int link_count_for_wap(SiteBean site){
		if(site == null)
			return -1;
		return LinkDAO.getLinkCountOfWapSite(site);
	}
