...
public class DLOG_Diary_VelocityTool {
	...
	public List list_hot_articles(SiteBean site, int days, int count) {
		if (site == null)
			return null;
		List articles = DiaryDAO.listHotArticles(site.getId(), days, count);
		if (articles == null || articles.size() == 0) {
			articles = DiaryDAO
					.listHotArticles(site.getId(), days * 100, count);
		} else if (articles.size() < count) {
			List others = DiaryDAO.listHotArticlesBefore(site.getId(), days,
					count - articles.size());
	...
	public DiaryReplyBean reply(int reply_id) {
		if (reply_id < 0)
			return null;
		return (DiaryReplyBean) ReplyDAO.getReply(DiaryReplyBean.class,
				reply_id);
	}
	...
	public _DiaryBase diary(SiteBean site, SessionUserObject user, int log_id) {
		...
		String text = TextCacheManager.getTextContent(DiaryBean.TYPE_DIARY,
				log_id);
		_DiaryBase diary = null;
		if (text == null) {
			diary = DiaryDAO.getDiaryByID(log_id);
			if (diary != null && diary.getStatus() == DiaryBean.STATUS_NORMAL) {
				TextCacheManager.updateTextContent(DiaryBean.TYPE_DIARY,
						log_id, diary.getContent());
			}
		} else {
	...
	public void fill_diary_content(_DiaryBase diary) {
		...
		if (text == null) {
			...
			if (diary != null && diary.getStatus() == DiaryBean.STATUS_NORMAL) {
				TextCacheManager.updateTextContent(DiaryBean.TYPE_DIARY, diary
						.getId(), diary.getContent());
			}
		} else {
			diary.setContent(text);
		}
	 ...
	 * @return
	 */
	public DiaryOutlineBean prev_diary(SiteBean site, SessionUserObject user,
			int catalog_id, int log_id) {
		if (site == null || log_id < 0)
			return null;
		try {
	 ...
	 * @return
	 */
	public DiaryOutlineBean next_diary(SiteBean site, SessionUserObject user,
			int catalog_id, int log_id) {
		if (site == null || log_id < 0)
			return null;
		try {
	 ...
	 * @return
	 */
	public List top_diary(SiteBean site, SessionUserObject user, int page,
	 ...
	 * @return
	 */
	public List list_diary(SiteBean site, SessionUserObject user,
			int catalog_id, int year, int month, int date, int page,
			int pageSize) {
		return list_diary(site, user, catalog_id, year, month, date, page,
				pageSize, true);
	}
	...
	public List list_diary(SiteBean site, SessionUserObject user,
			int catalog_id, int year, int month, int date, int page,
			int pageSize, boolean withContent) {
		...
		if (pageSize < 1)
			pageSize = 5;
		if (site == null)
			return DiaryDAO.listDiary(year, month, date, fromIdx, pageSize,
					withContent);
		else
			return DiaryDAO.listDiary(site, user, catalog_id, year, month,
					date, fromIdx, pageSize, withContent);
	}
	 ...
	 * @return
	 */
	public int diary_count(SiteBean site, SessionUserObject user,
			int catalog_id, int year, int month, int date) {
		if (site == null)
			return -1;
		return DiaryDAO
				.getDiaryCount(site, user, catalog_id, year, month, date);
	}
	 ...
	 * @param pageSize
	 * @param reverse
	 *            是否倒序,如果该值为true则最新评论排在最前面
	 * @return
	 */
	public List replies(SiteBean site, SessionUserObject user,
			_DiaryBase diary, int page, int pageSize, boolean reverse) {
		if (site == null)
			return null;
		if (diary == null || diary.getSite().getId() != site.getId())
			return null;
		...
		if (fromIdx < 0)
			fromIdx = 0;
		return DiaryDAO.listDiaryReplies(diary.getId(), fromIdx, pageSize,
				reverse);
	}
	...
	private final static String new_rpls_cache_key = "query.new_replies_of_site";
	 ...
	 * @return
	 */
	public List list_diary_replies(SiteBean site, int page, int count,
		...
		nKey.append('_');
		nKey.append((page > 0) ? page : 0);
		nKey.append('_');
		nKey.append((count > 0) ? count : 0);
		if (user != null) {
			nKey.append('_');
			nKey.append(user.getId());
		}
		List rpls = (List) DLOG_CacheManager.getObjectCached(
				new_rpls_cache_key, nKey.toString());
		if (rpls == null) {
			int fromidx = -1;
			if (page > 0 && count > 0)
				fromidx = (page - 1) * count;
			rpls = DiaryDAO.listDiaryReplies(site, fromidx, count, user);
			if (rpls != null)
				DLOG_CacheManager.putObjectCached(new_rpls_cache_key, nKey
						.toString(), rpls);
		}
		return rpls;
	}
	 ...
	 */
	public List diary_counts_by_month(SiteBean site, SessionUserObject user,
			int year, int month) {
		if (site == null)
			return null;
		...
		List<Integer> logcs = new ArrayList<Integer>();
	 ...
	 * @return
	 */
	public List list_trash(SiteBean site, SessionUserObject user, int page,
			int pageSize) {
		if (site == null || user == null)
			return null;
		if (!site.isOwner(user))
	...
	public List list_drafts(SiteBean site, SessionUserObject user, int page,
			int pageSize) {
		return list_drafts(site, user, page, pageSize, true);
	}
	 ...
	 * @param log
	 */
	public void visit_diary(SiteBean site, SessionUserObject user,
			_DiaryBase diary) {
		if (diary != null) {
			try {
	 ...
	 * @return
	 */
	public List list_special_popedom_users_by_catalog(SiteBean site,
			SessionUserObject user, CatalogBean catalog) {
	 ...
	 * 显示上传附件列表
	 */
	public String annex_table(String id, String vid) {
		try {
			//System.out.println("调试: annex_table() vid="+vid);
			List annexList = AnnexDAO.nonceAnnex(Integer.parseInt(id), Long
					.parseLong(vid));
			if (annexList.size() < 1)
				return "NULL";
			...
			String fmt = null;
			long validate = 0;
			fmt = "yyyy年M月d日 EEEEE HH:mm:ss";
			buf.append("<table id=\"annex\" name=\"annex\" >");
			for (int i = 0; i < annexList.size(); i++) {

				bean = (AnnexBean) annexList.get(i);
				String time = new SimpleDateFormat(fmt).format(bean
						.getUploadTime());
				buf.append("<tr>");
				buf.append("<td title=\"上传时间:" + time + "\">");
				buf.append("附件名称:[" + bean.getFileName() + "]&nbsp");
				buf.append("</td>");
				buf.append("<td>");
				String fs = new DecimalFormat("#########.##")
						.format(((float) bean.getFileSize() / (float) 1024));
				buf.append("大小:" + fs + "k");
				buf.append("</td>");
				buf
						.append("<td><a href=\"javascript:deleteAnnex("
								+ bean.getId() + ")\">删除</a>");
				buf.append("</td>");
				buf.append("</tr>");
				countSize += bean.getFileSize();
				validate = bean.getValiDate();
			}
			buf.append("</tr>");
			String fsc = new DecimalFormat("#########.##")
					.format((float) countSize / (float) 1024);
			buf.append("<td >总大小:" + fsc + "K");
			buf.append("</td></tr>");
			buf.append("</table>");
			return buf.toString();
		} catch (Exception ex) {
	 ...
	 * 显示日志附件列表
	 */
	public String annex_table(int diaryID) {
		try {
			
			List annexList = AnnexDAO.nonceAnnex(diaryID);
//			if (annexList.size() < 1)
//				return "";
			StringBuffer buf = new StringBuffer();
			AnnexBean bean;
			...
			String lasttime = "";
			long validate = 0;
			fmt = "yyyy年M月d日 HH:mm:ss";
			buf.append("<table id=\"annex\" name=\"annex\" >");
			for (int i = 0; i < annexList.size(); i++) {
				bean = (AnnexBean) annexList.get(i);
				// System.out.println(bean.getLastDownload()==null);
				time = new SimpleDateFormat(fmt).format(bean.getUploadTime());
				if (bean.getLastDownload() != null)
					lasttime = new SimpleDateFormat(fmt).format(bean
							.getLastDownload());
				buf.append("<tr>");
				buf.append("<td title=\"上传时间:" + time);
				buf.append("&#13最后下载时间:" + lasttime + "&#13下载次数:"
						+ bean.getDownloadCount() + "\" >");
				buf.append("附件名称:[<a href=\"javascript: download("
						+ bean.getId() + ");" + "\">" + bean.getFileName()
						+ "</a>]&nbsp");
				buf.append("</td>");
				buf.append("<td>");
				String fs = new DecimalFormat("#########.##")
			...
			if (validate == 0) {
				String vali = "yyyyMMddHHmmss";
				validate = Long.parseLong(new SimpleDateFormat(vali)
						.format(new Date()));
				buf.append("<input type=\"hidden\" id=\"valiDate\" value=\""
						+ validate + "\"/>");
			} else {
				buf.append("<input type=\"hidden\" id=\"valiDate\" value=\""
						+ validate + "\"/>");
	 ...
	 public long annex_validate(int diaryID) {
		 return AnnexDAO.annex_validate(diaryID);
	 }
