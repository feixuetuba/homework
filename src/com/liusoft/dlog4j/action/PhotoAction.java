...
public class PhotoAction extends AdminActionBase {
			...
			final HttpServletResponse response, String s_photo_id) throws Exception 			
	{
		...
		do{
			...
			List<String> vtMails = new ArrayList<String>();
	...
	protected void sendMails(final HttpServletRequest request,
			final SessionUserObject loginUser, final _PhotoBase pbean, final List<String> mails)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	protected ActionForward doImport(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) throws Exception 			
	{
		...
		do{
			//检查相簿是否有效
			AlbumBean album = AlbumDAO.getAlbumByID(photo.getAlbum());
			if (album == null) {
				msgs.add("import", new ActionMessage("error.object_not_found", photo.getAlbum()));
				break;
			}

			SiteBean site = super.getSiteByID(photo.getSid());
			if(site == null)
				site = album.getSite();

			String photo_desc;
			if(StringUtils.isNotEmpty(photo.getDesc())){
				photo_desc = StringUtils.abbreviate(super.autoFiltrate(
						site, photo.getDesc()), MAX_PHOTO_DESC_LENGTH);
			...
			if(!ImageUtils.isImage(path)){
				msgs.add("import", new ActionMessage("error.upload_file_not_supported"));
				break;
			}
			File imgFile = new File(path);
			
			//检查上传空间限制
			int photo_size = DLOG4JUtils.sizeInKbytes(imgFile.length());
			...
			if(max_photo_size >= 0){				
				...
				if((current_size + photo_size) > max_photo_size){
					msgs.add("import", new ActionMessage("error.photo_space_full"));
					break;
				}
			}
			//保存照片并生成略缩图
			...
			try{
				img = getPhotoSaver().save(
						...
						imgFile, photo.getAutoRotate() == 1);
				if(img == null){
					msgs.add("import", new ActionMessage("error.upload_failed"));
					break;				
				}
				//写入到数据库
				PhotoBean pbean = new PhotoBean();
				pbean.setSite(site);
				pbean.setUser(site.getOwner());
				if(StringUtils.isNotEmpty(photo.getName())){
					pbean.setName(super.autoFiltrate(site, photo.getName()));
				}
				else
					pbean.setName(img.getName());
				pbean.setDesc(photo_desc);
				if(StringUtils.isNotEmpty(photo.getKeyword())){
					if(site.isFlagSet(SiteBean.Flag.ILLEGAL_GLOSSARY_IGNORE))
						pbean.setKeyword(photo.getKeyword());
					else
						pbean.setKeyword(DLOGSecurityManager.IllegalGlossary
								.deleteIllegalWord(photo.getKeyword()));
				}
				...
				pbean.setStatus(photo.getStatus());//公开或者隐藏
				PhotoDAO.create(album, pbean, (photo.getCover()==1));
			}catch(IllegalAccessException e){
				msgs.add("import", new ActionMessage("error.access_deny", e.getMessage()));
				break;
			}catch(Exception e){
				log.error("Upload photo file failed.", e);
				msgs.add("import", new ActionMessage("error.upload_failed", e.getMessage()));
				break;
			}
			break;
		}while(true);
		...
		if(!msgs.isEmpty()){
			saveMessages(request, msgs);
			ActionForward upload = makeForward(mapping.findForward("photo_upload"), photo.getSid());
			upload.setRedirect(true);
			return upload;
		}
		
		String ext = "aid=" + photo.getAlbum();
		return makeForward(mapping.findForward("photo_album"), photo.getSid(), ext);
	}
	/**
	 * 上传照片
			...
			final HttpServletResponse response) throws Exception 			
	{
		...
		super.validateClientId(request, photo1);
