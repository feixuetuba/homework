...
public abstract class ActionBase extends ActionExtend {
	...
	protected boolean accept(FormFile file){
		return ImageUtils.isImage(file.getFileName());
	}
	...
	private final static NodeFilter scriptFilter = new NodeFilter() {
		public boolean accept(Node node) {
			if(node instanceof Tag){
				Tag tag = (Tag)node;
				if("embed".equalsIgnoreCase(tag.getTagName()))
					return false;
			}
	...
	protected static SessionUserObject getLoginUser(HttpServletRequest request,
		...
		//如果无法通过session或者cookie获取用户资料则试图通过uuid参数去获取(Add by Winter Lau , 2007-3-15)
		if(user == null)
			user = getUserByUUID(request, response); 
	...
	/**
	 * 实现通过uuid参数进行自动登录,用于wap的书签自动登录
	 * @param request
	 * @param response
	 * @return
	 */
	protected static UserBean getUserByUUID(HttpServletRequest request,	HttpServletResponse response){
	...
	protected static ActionForward makeForward(ActionForward forward, String p_name,
			int p_value) {
		return makeForward(forward, p_name, String.valueOf(p_value));
	}
