...
public class ReplyAction extends ActionBase {
	...
	protected ActionForward doAddDiaryReply(ActionMapping mapping,
		...
		do{
			...
			if(loginUser==null){
				//验证校验码
				if (!StringUtils.equals(reply.getVerifyCode(), ReplyForm.getVerifyCode(request))){
					msgs.add("reply", new ActionMessage("error.verified_failed"));
					break;
				}
			}
			//Modified by Winter Lau (2007-4-10)
			if (StringUtils.isNotBlank(reply.getAuthor())
					&& (loginUser == null || !StringUtils.equalsIgnoreCase(
							reply.getAuthor().trim(), loginUser.getNickname()))) {
				//检查评论者输入的用户名是否已存在
				if(UserDAO.getUserByNickname(reply.getAuthor().trim())!=null){
					msgs.add("reply", new ActionMessage("error.reply_author_exists"));
					break;
				}
			}
			SiteBean site = super.getSiteByID(reply.getSid());
			...
			rbean.setAuthor(getAuthor(request, reply));
			if (StringUtils.isNotBlank(reply.getAuthorURL()))
				rbean.setAuthorURL(reply.getAuthorURL());
			if (StringUtils.isNotBlank(reply.getAuthorEmail()))
				rbean.setAuthorEmail(reply.getAuthorEmail());
	...
	private String getAuthor(HttpServletRequest req, ReplyForm rform){
		if(StringUtils.isNotBlank(rform.getAuthor()))
			return super.autoFiltrate(null,rform.getAuthor());
		else{
